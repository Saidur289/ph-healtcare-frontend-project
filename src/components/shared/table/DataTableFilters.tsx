...
const getFilterActiveCount = (
  ...
  if (isRangeValue(value)) {
    return Object.values(value).filter((item) => item.length > 0).length;
  }
  return 0;
};
...
const RangeFilterControl = ({
  ...
  const applyNow = () => {
    const hanAnyValue = RANGE_OPERATORS.some((operator) =>
      rangeValue[operator]?.trim(),
    );
    onFilterChange(filter.id, hanAnyValue ? rangeValue : undefined);
  };
  const clearRange = () => {
    setRangeValue({});
          ...
          {activeBadges.map((badge) => (
            ...
            >
              ...
              <Button
                ...
                disabled={isLoading}
                className="ml-0.5 rounded-full p-0.5 hover:bg-muted-foreground/20 disabled:pointer-events-auto"
