...
import {
  ...
  type IDoctorDetails,
  type IDoctorReview,
  type IDoctorScheduleItem,
  type IDoctorAppointmentItem,
...
const ViewDoctorProfileDialog = ({
  ...
  const isApiResponse = (
    response: unknown,
  ): response is ApiResponse<IDoctorDetails> => {
    return (
      !!response &&
      typeof response === "object" &&
      "data" in response &&
      (response as unknown as Record<string, unknown>).success === true
    );
  };
  const doctorDetails = isApiResponse(data) ? data.data : null;
                  ...
                  <div className="rounded-lg border p-4">
                    ...
                    <div className="space-y-2">
                      {(doctorDetails.reviews ?? [])
                        .slice(0, 5)
                        .map((review: IDoctorReview, index: number) => (
                          <div
                ...
                <div className="rounded-lg border p-4">
                  ...
                  <div className="space-y-2">
                    {(doctorDetails.appointments ?? [])
                      ...
                      .map(
                        (
                          appointment: IDoctorAppointmentItem,
                          index: number,
                        ) => (
                          <div
